...
class HybridAIImageService:
    ...
    def _analyze_description_quality(self, description: str) -> Dict[str, Any]:
        ...
        # Content checks
        venue_words = ['venue', 'location', 'setting', 'space', 'center', 'hall', 'arena', 'theatre', 'club', 'restaurant']
        ...
        visual_words = ['beautiful', 'stunning', 'elegant', 'vibrant', 'modern', 'traditional', 'spectacular', 'intimate']
    ...
    async def generate_image(self, event: Dict[str, Any]) -> Optional[str]:
        """Generate AI image using hybrid approach"""
        
        event_title = event.get('title', 'Unknown Event')
        logger.info(f"🎨 Generating hybrid AI image for: {event_title}")
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(f"✅ Successfully generated hybrid image for: {event_title}")
                        return image_url
